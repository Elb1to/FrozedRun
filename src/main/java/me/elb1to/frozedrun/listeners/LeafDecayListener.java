...
public class LeafDecayListener implements Listener {
    ...
    @EventHandler
    public void breakWood(BlockBreakEvent event) {
        ...
        if (material == Material.LOG || material == Material.LOG_2) {
                    ...
                    for (int i = 0; i < blockList.size(); i++) {
                        ...
                        for (BlockFace blockFace : BlockFace.values()) {
                            if (block.getRelative(blockFace).getType() == Material.LOG || block.getRelative(blockFace).getType() == Material.LOG_2) {
                                blockList.add(block.getRelative(blockFace));
                            }
                        }
            ...
            }.runTaskTimer(FrozedUHCRun.getInstance(), 1, 1);
        }
    }
    ...
    private void checkLeaves(Block block) {
        ...
        Bukkit.getServer().getScheduler().runTask(FrozedUHCRun.getInstance(), new Runnable() {
